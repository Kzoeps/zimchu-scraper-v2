...
def is_facebook_listing_valid(post_url):
    ...
    try:
        ...
        if response.status_code == 200 and "content isn't available" not in response.text.lower() and "this content isn't available" not in response.text.lower():
            return True
        return False
    ...
    for listing in response.data:
        ...
        if not is_facebook_listing_valid(post_url):
            ...
            try:
                ...
                delete_response = supabase.table("listings_v2").delete().eq("id", listing_id).execute()
...
def delete_old_listings(weeks=3):
    ...
    response = supabase.table("listings_v2").select("id", "created_at").lt("created_at", cutoff_date).execute()
    ...
    if failed_listings:
        print(f"Failed to delete {len(failed_listings)} old listings: {failed_listings}")
